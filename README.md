...
A production-minded Python backend for **versioned threat scoring, durable retraining orchestration, and safe model promotion**.

The classifier is one component, not the architecture. The engineering focus is the contract around it: bounded request and inference admission, durable idempotent jobs, worker leases, immutable model bundles, checksum and schema verification, last-known-good serving, explicit health semantics, observability, container isolation, and documented failure boundaries.
...
4. [`networksecurity/services/model_registry.py`](networksecurity/services/model_registry.py) — immutable model/preprocessor bundles, SHA-256 verification and atomic `current.json` promotion.
5. [`networksecurity/services/training_jobs.py`](networksecurity/services/training_jobs.py) + [`training_worker.py`](networksecurity/services/training_worker.py) — durable idempotency, bounded backlog, worker leases, heartbeats, retries and crash recovery.
...
7. [`docs/DESIGN_REVIEW.md`](docs/DESIGN_REVIEW.md) — the five design decisions that carry most of the backend-system reasoning.
...
flowchart TB
    ...
    Registry["Immutable Model Registry<br/>versions/&lt;id&gt;/<br/>manifest + SHA-256<br/>current.json"]:::registry
    ...
    subgraph CP["CONTROL / TRAINING PLANE"]
        ...
        Queue["SQLite durable job store<br/>idempotency constraint<br/>bounded backlog<br/>worker lease + attempts<br/>crash recovery"]:::control
        Worker["Training Worker<br/>separate process"]:::control
        Pipeline["ingest → validate → transform<br/>cross-validate → held-out gate<br/>publish immutable bundle"]:::control
        Mongo[("MongoDB<br/>training source")]:::external
        S3[("S3<br/>optional artifact mirror")]:::external

        Admin -->|POST /api/v1/training-jobs| Queue
        Queue -->|claim + lease| Worker
        Mongo --> Worker
        Worker --> Pipeline
        Pipeline -. optional mirror .-> S3
    end

    Pipeline -->|promote accepted version| Registry
```
...
- **Model publication is single-writer in this deployment.** Competing workers may poll the queue, but only one unexpired training lease may be `RUNNING` at a time.
...
```mermaid
flowchart TB
    Q["QUEUED"] -->|worker claim + lease| R["RUNNING"]
    R -->|accepted model published| S["SUCCEEDED"]
...
`current.json` is intentionally tiny. Publication commits the immutable version first and replaces the pointer last. Optional S3 synchronization follows the same ordering: version contents first, pointer last.
...
This keeps MongoDB clients, MLflow, boto3 and training orchestration out of the request-serving image and reduces runtime coupling and attack surface.
...
To seed the example training dataset into MongoDB:

```bash
python scripts/seed_mongodb.py --replace
```

The default local database and collection are `networksecurity.network_events`; both can be overridden through environment variables.
...
GitHub Actions installs the project itself, compiles production code, runs correctness-focused Ruff rules, executes branch-aware coverage with a 75% floor, verifies dependency consistency, audits serving dependencies and builds both non-root images with smoke-import checks.
...
python -m pytest --cov=networksecurity.api --cov=networksecurity.services --cov=networksecurity.config
...
- mutable `final_model/` files became immutable version bundles plus an atomic promotion pointer
- process-local training state became restart-safe SQLite idempotency and leases
...
- **SQLite is not a proposed multi-region control plane.** It demonstrates durable idempotency, leases, crash recovery and process separation on one host/shared filesystem. A real multi-node deployment should move the job contract behind PostgreSQL plus a durable queue, or an equivalent managed design.
...
The current contracts are shaped so infrastructure can be replaced without rewriting clients: `TrainingJobStore` can become PostgreSQL plus SQS/Kafka-backed orchestration; `ModelRegistry` can become an object-store registry; the refresh loop can become event-driven promotion notification; admin authentication can move to workload identity; and serving replicas can remain stateless apart from their immutable in-memory model snapshot.

See [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md), [`docs/OPERATIONS.md`](docs/OPERATIONS.md), [`docs/FAILURE_MODES.md`](docs/FAILURE_MODES.md), [`docs/DESIGN_REVIEW.md`](docs/DESIGN_REVIEW.md), and the ADRs under [`docs/adr/`](docs/adr/) for the reasoning and failure trade-offs.
...
docs/                        # architecture, operations and design decisions
```

## Scope

The phishing dataset is the workload used to exercise the backend architecture. This repository is not presented as a production IDS/IPS or SOC product. Its purpose is to demonstrate how a model-backed decision service can be explicit about **concurrency, durability, versioning, promotion, rollback, observability and failure behavior**.
